...
Walk-forward analysis is the practitioner's standard for strategy validation because it mimics the actual deployment environment. The ratio $SR_{\text{OOS}} / SR_{\text{IS}}$ (out-of-sample to in-sample Sharpe) is an informal measure of **Sharpe preservation** — values below 0.5 indicate severe overfitting; values above 0.75 suggest robust generalisation. The permutation test can be applied to the out-of-sample returns to confirm that the residual OOS Sharpe is statistically significant and not a lucky streak, combining walk-forward and permutation testing into a robust validation framework.
