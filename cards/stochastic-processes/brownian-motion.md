...
- The $(dW)^2 = dt$ result is crucial — more precisely, the **quadratic variation** of $W$ over $[0,t]$ equals $t$ almost surely, so in an $L^2$ sense $(dW)^2 = dt$. This is not a pathwise equality but it means the second-order randomness is effectively deterministic, which is what makes Itô's lemma tractable.
