...
For a dampened call price $c_T(k) = e^{-\alpha k} C(k)$ at log-strike $k = \ln K$, the Fourier transform is:

$$\psi_T(v) = \frac{e^{-rT} \phi_T(v - (\alpha+1)i)}{\alpha^2 + \alpha - v^2 + i(2\alpha+1)v}$$

Where $\phi_T$ is the model's characteristic function and $\alpha > 0$ is a damping constant. The call prices at a grid of $N$ log-strikes are recovered by the inverse FFT:
...
def carr_madan(phi, S, r, T, alpha=1.5, N=4096, eta=0.25):
    ...
    k = -np.pi / eta + (2 * np.pi / (N * eta)) * np.arange(N)  # log-strike grid
